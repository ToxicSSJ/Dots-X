...
public class InfiniteTimeline extends GameTimeline implements ConsoleTimeline, Serializable {
	
	private static final long serialVersionUID = -3034521677046714162L;

	private static transient InfiniteTimeline instance;
	...
	private InfiniteTimeline(LinkedList<String> data) {
	...
	public void start() {
		...
		ConsoleAdapter.getAdapter().getWriter().addLayer(
				new AssemblerLayer("GameAssembler", "assets/console/layers/games/X_GAME_NON")
					.autoFrameUp(50)
	...
	public void show() {
		...
		timer.schedule(new TimerTask() {
			...
			public void run() {
				...
				game = new VariableLayer("Game", "assets/console/layers/games/X_GAME")
						.attach(variables)
						...
						if(input.startsWith(Configuration.get("hotkey_p"))) {
							...
							if(selection == false) {
								...
								if(selectionDots.size() > 1) {
									...
									scoreint += selectionDots.size() * 30;
				...
				board = new VariableLayer("Score", "assets/console/layers/games/X_GAME_SCORE")
							.addModifier(new LayerModifier() {
	...
	public void finish() {
		...
		WinTimeline.getModeInstance().initDefault("INFINITO", scoreint, starsint);
		return;
	...
	public static InfiniteTimeline getModeInstance() {
		return instance;
	}
	
	public static InfiniteTimeline continueGame(LinkedList<String> data) {
		return new InfiniteTimeline(data);
	}
