...
public class OriginalTimeline extends GameTimeline implements ConsoleTimeline, Serializable {
	
	private static final long serialVersionUID = -3034521677046714162L;

	private static transient OriginalTimeline instance;
	...
	protected Playlist music = new Playlist(MusicType.CONSOLE_ORIGINAL_MUSIC1, MusicType.CONSOLE_ORIGINAL_MUSIC2, MusicType.CONSOLE_ORIGINAL_MUSIC3);
	...
	protected int movesint = 0;
	protected int movesmaxint = 30;
	...
	public OriginalTimeline() {}
	
	private OriginalTimeline(LinkedList<String> data) {
		...
		for(String line : data) {
			...
			if(listKey != "none") {
				...
				if(listKey == "DOTS")
					dots.add(Variable.fromString(line));
			...
			if(line.startsWith("MOVES: ")) {
				...
				movesint = Integer.parseInt(line.replaceFirst("MOVES: ", ""));
				continue;
			...
			if(line.startsWith("MAX: ")) {
				...
				movesmaxint = Integer.parseInt(line.replaceFirst("MAX: ", ""));
				continue;
		...
		moves = new Variable("%mov", String.format("%04d", movesint));
		maxmoves = new Variable("%max", String.format("%04d", movesmaxint));
	...
	public void start() {
		...
		for(char i = 'A'; i <= 'Y'; i++)
			dots.add(new Variable("%dot" + i, AnsiConverter.random()));
		...
		ConsoleAdapter.getAdapter().getWriter().addLayer(
				new AssemblerLayer("GameAssembler", "assets/console/layers/games/ORIGINAL_GAME_NON")
					.autoFrameUp(50)
	...
	public void show() {
		...
		timer.schedule(new TimerTask() {
			...
			public void run() {
				...
				game = new VariableLayer("Game", "assets/console/layers/games/ORIGINAL_GAME")
						.attach(variables)
						...
						if(input.startsWith(Configuration.get("hotkey_p"))) {
							...
							if(selection == false) {
								...
								if(selectionDots.size() > 1) {
									...
									movesint++;
									moves.silentChange(String.format("%04d", movesint));
				...
				board = new VariableLayer("Score", "assets/console/layers/games/ORIGINAL_GAME_SCORE")
							.addModifier(new LayerModifier() {
	...
	public void replace() {
		...
		for(Variable dot : dots)
			if(dot.getValue().equals("&x"))
				dot.silentChange(AnsiConverter.random());
	...
	public LinkedHashSet<Entry<Integer, Integer>> getCorner(int dotid) {
		...
		corner.add(getCornerPos(index.getKey(), (index.getValue() + 11)));
		corner.add(getCornerPos(index.getKey() + 1, (index.getValue() + 11)));
	...
	public static OriginalTimeline getModeInstance() {
		return instance;
	}
	
	public static OriginalTimeline continueGame(LinkedList<String> data) {
		return new OriginalTimeline(data);
	}
	...
	public String toData() {
		...
		data.append("#----------------------------#\n")
			...
			.append("#----------------------------#\n")
			.append("MODO: ORIGINAL\n")
			.append("FECHA: " + TimeFormatter.getAdvancedCurrentTime() + "\n")
			.append("SCORE: " + scoreint + "\n")
			.append("STARS: " + starsint + "\n")
			.append("MOVES: " + movesint + "\n")
			.append("MAX: " + movesmaxint + "\n")
